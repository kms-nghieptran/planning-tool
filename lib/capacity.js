...
function memberRow(member, row, work, s) {
  ...
  /* AN EXEMPT MEMBER CONTRIBUTES NO HOURS, exactly as a released one does —
     the row stays, with their own days and their own work on it, and only the
     capacity arithmetic stops counting them. */
  const counted = member.status !== 'Released' && !member.calcExempt;
  const hours = counted ? capacityHours(days, member.supportPct, s) : 0;
...
function sprintGrid(team, sprint, avail, workBy) {
  ...
  /* WHO COUNTS TOWARDS CAPACITY IS NOT WHO COUNTS TOWARDS COMMITMENT.
     `active` is the capacity side — headcount, available days, hours, and the
     predicted points derived from them. A released member is out of it, and so
     now is a calc-exempt one: someone on loan to another team, or a lead whose
     sprint is meetings, is on the roster but is not capacity you can plan
     against, and counting them inflates the number the plan is measured by.

     `planned` and `actual` stay over ALL rows, deliberately. Committed and
     delivered points are facts about the sprint's work, not about whose hours
     paid for it — the Committed KPI and the burndown both read them, and
     dropping an exempt member's committed work would make the burndown start
     below the work that actually has to get done. Exempting someone removes
     their capacity; it never removes their tickets. */
  const active = rows.filter(r => r.status !== 'Released' && !r.calcExempt);
  ...
  return {
    ...
    totals: {
      ...
      exempt: rows.filter(r => r.calcExempt).length,
