...
/** Resolve a Jira assignee onto a team member. accountId wins; then name aliases. */
function memberResolver(team) {
  ...
  return (issue) => {
    ...
    if (issue.assignee && byName.has(norm(issue.assignee))) return byName.get(norm(issue.assignee));
    return null;
  };
}
...
function workForSprint(plan, snap, team, sprint, raw = null) {
  ...
  for (const i of issues) {
    const bucket = i.memberId && byMember[i.memberId] ? byMember[i.memberId] : unassigned;
    const pts = Number(i.points) || 0;
    bucket.planned += pts;
    if (isDone(i)) bucket.actual += pts;
    bucket.items.push(i);
  }
  ...
  return { issues, byMember, unassigned };
}
...
function capacityView(plan, snap, baseTeam, rawSprint) {
  ...
  const team = teamForSprint(plan, baseTeam, sprint, roster);
  ...
  grid.roster = {
    ...
    members: team.members.map(m => ({
      id: m.id, name: m.name, onSprint: m.onSprint || 'added',
  ...
  grid.unassigned = { points: round1(work.unassigned.planned), count: work.unassigned.items.length, items: work.unassigned.items };
...
function activeSprintView(plan, snap, team, rawSprint, { today = new Date() } = {}) {
  ...
  const unassignedDone = round1(work.unassigned.items.filter(isDone).reduce((t, i) => t + (Number(i.points) || 0), 0));
  const committed = round1(grid.totals.planned + grid.unassigned.points);
  const done = round1(grid.totals.actual + unassignedDone);
  ...
  return {
    ...
    burndown,
    health: sprintHealth({ timeElapsedPct, donePct, grid, blocked, unestimated, unassignedPoints: grid.unassigned.points }),
    items,
  };
}
...
function sprintHealth({ timeElapsedPct, donePct, grid, blocked, unestimated, unassignedPoints }) {
  const reasons = [];
  let score = 100;
...
function velocityHistory(plan, snap, team) {
  return plan.sprints.slice().sort(reconcile.compareSprints).map(raw => {
    const sprint = reconcile.forTeam(raw, team.id);
    const t = teamForSprint(plan, team, sprint);
    const avail = availabilityFor(plan, t, sprint);
    const work = workForSprint(plan, snap, t, sprint);
    ...
    return {
      ...
      planned: grid.totals.planned,
      actual: grid.totals.actual,
