...
function testCaseSummary(items, lookup = () => null) {
  ...
  for (const i of items || []) {
    ...
    const maintained = bucket
      ...
      : [];
    for (const k of maintained) note(k, lookup(k), 'test case');
  ...
  const out = [...rows.values()]
    .map(r => ({
      ...
      keys: {
        automated: [...r._automated], inFlight: [...r._inFlight], maintained: [...r._maintained],
  ...
  const totalKeys = {
    automated: [...all.automated], inFlight: [...all.inFlight], maintained: [...all.maintained],
  ...
  return {
    ...
    // Maintained. `absent` marks one this tool has no local copy of.
