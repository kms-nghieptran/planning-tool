  ...
  const issues = (raw || issuesForSprint(snap, team, sprint))
    .map(i => {
      ...
      /* WHAT IS HOLDING THIS UP IS RECORDED ON THE EPIC, NOT ON THE STORY.
         ...
         Attached here, beside `epics`, because it is the same question one
         step on: `epicsFor` has already decided which epic is behind this work
         — parent for a Story, a "relates to" link for a maintenance ticket —
         and asking that again anywhere else is how two screens come to
         disagree about which epic a row belongs to. Only epics that actually
         name something are kept, so an empty array means "nothing recorded",
         never "not looked up". */
      out.epicBlockers = (out.epics || [])
        .map((e) => {
          const epic = lookup(e.key);
          /* The whole link, not just its key. Every blocker in his store lives
             in a project this tool does not sync, so the summary Jira sent
             inside the link is the only description of it that will ever exist
             locally — keep it and the drawer can name the thing in the way
             instead of printing a bare key under "not in the local store". */
          const seen = new Set();
          const blockers = ((epic && epic.blockedBy) || [])
            .map(b => epicsLib.asLink(b))
            .filter(l => l.key)
...
function teamForSprint(plan, team, sprint, roster = null) {
  ...
  const members = base.map(m => {
    ...
    return exempt ? { ...out, calcExempt: true } : out;
  });
...
function testCaseSummary(items, lookup = () => null) {
  ...
  const note = (key, issue, kind) => {
    ...
    catalogue.set(k, issue
      ? { key: k, summary: issue.summary || '', status: issue.status || '', statusCategory: issue.statusCategory || '', type: issue.issueType || '', kind }
      : { key: k, summary: '', status: '', statusCategory: '', type: '', kind, absent: true });
  };
  ...
  const add = (o) => signals.push({ teamId: team.id, teamName: team.name, sprintId: current.id, ...o });
  {
    ...
    if (view.progress.blocked.count) add({
      ...
      detail: view.progress.blocked.items.slice(0, 6)
        .map(i => `${i.key} ← ${(i.blockedBy || []).map(b => epicsLib.asLink(b).key).filter(Boolean).join(', ')}`).join('; '),
...
module.exports = {
  ...
  snapToWeeks, fitRow,
  capacityView, backlogView, sprintIssueKeys, activeSprintView, forecastView, riskView, signalsFor, teamForSprint,
