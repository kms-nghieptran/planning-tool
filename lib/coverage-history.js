 ...
 */
function movement(component = null, { scope = 'Epic', since = null, limit = 400, exclude = null } = {}) {
  ...
  return {
    ...
    movers: component == null ? movers({ scope, since, limit, exclude }) : [],
  };
}
 ...
 */
function movers({ scope = 'Epic', since = null, limit = 400, min = 1, exclude = null } = {}) {
  /* HIS EXCLUDED COMPONENTS DO NOT GET A ROW.
     They are filtered out here rather than in the route because this is where
     the list of names is decided, and because the readings themselves must stay
     — a component he excludes today still has a recorded past, and un-excluding
     it has to bring that past back rather than start the curve from scratch.
     The table above this one already hides them; a mover row for a component
     that has no row up there is the two halves of one screen disagreeing. */
  const dropped = cov.excludeSet(exclude);
  const names = db.all(
    'SELECT DISTINCT component FROM coverage_reading WHERE scope = ? AND component != ?', scope, ALL,
  ).map(r => r.component).filter(n => !dropped.has(String(n || '').trim().toLowerCase()));
