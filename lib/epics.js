 ...
 */
function maintainedCount(issue) {
  if (!issue) return 0;
  const keys = new Set();
  for (const l of issue.relatesTo || []) {
    const k = asLink(l).key;
    if (k) keys.add(String(k).trim().toUpperCase());
  }
  return keys.size;
}

module.exports = { epicsFor, rollup, isEpicType, EPIC_TYPE, isBucketStory, maintainedCount, BUCKET_TYPE };
