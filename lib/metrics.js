...
function velocity(plan, snap, team, { sprints = 12 } = {}) {
  const history = insights.velocityHistory(plan, snap, team)
    .filter(h => h.planned > 0 || h.actual > 0)
    .slice(-sprints);

  const completed = history.filter(h => h.actual > 0);
  const avg = cap.averageVelocity(history, 6);
...
function productivity(plan, snap, team, { sprints = 12 } = {}) {
  const history = insights.velocityHistory(plan, snap, team).filter(h => h.actual > 0).slice(-sprints);
...
function quality(plan, snap, team, { sprints = 12 } = {}) {
  const history = insights.velocityHistory(plan, snap, team).filter(h => h.planned > 0).slice(-sprints);
  const idx = (snap.byTeam || {})[team.id] || {};

  const items = [];
  for (const keys of Object.values(idx.sprintIssues || {})) {
    for (const k of keys) { const i = (snap.issues || {})[k]; if (i) items.push(i); }
  }
