 ...
 */
/* THE COLOUR TRAVELS WITH THE BUCKET, for the same reason a priority level
   carries its own: four screens render these, and a second list in a
   stylesheet is how Maintenance ends up pink on one of them and blue on
   another. `color` is the fill a bar or a dot takes; `ink` is the text-safe
   version for a coloured NUMBER on a card, which is a different measurement
   and therefore a different token — the same split `--cat-*` and `--st-*`
   already make in styles.css.

   N/A AND OBSOLETED USED TO BE THE SAME GREY, which meant two of the seven
   statuses were not distinguishable at all. Obsoleted takes purple — it is
   the bigger pile by far and the one that actually gets discussed — and
   "No Status" takes a plain dark neutral, which is the right look for the
   bucket that means nobody has triaged it yet. */
const BUCKETS = [
  { key: 'automated', label: 'Automated', inRatio: true, covered: true, color: 'var(--cov-automated)', ink: 'var(--cov-automated-ink)' },
  { key: 'maintenance', label: 'Maintenance', inRatio: true, covered: true, color: 'var(--cov-maintenance)', ink: 'var(--cov-maintenance-ink)' },
  { key: 'ready', label: 'Ready for Automation', inRatio: true, covered: false, color: 'var(--cov-ready)', ink: 'var(--cov-ready-ink)' },
  { key: 'blocked', label: 'Blocked', inRatio: true, covered: false, color: 'var(--cov-blocked)', ink: 'var(--cov-blocked-ink)' },
  { key: 'na', label: 'N/A for Automation', inRatio: false, covered: false, color: 'var(--cov-na)', ink: 'var(--cov-na-ink)' },
  { key: 'obsoleted', label: 'Obsoleted', inRatio: false, covered: false, color: 'var(--cov-obsoleted)', ink: 'var(--cov-obsoleted-ink)' },
  { key: 'none', label: 'No Status', inRatio: false, covered: false, color: 'var(--cov-none)', ink: 'var(--cov-none-ink)' },
];
...
/* The two automation tools, carried on the epic as components.
   ...
   THE TOOL IS NOT A HUE, IT IS A BAND. On a grid that shows both tools over
   all seven buckets, colouring the tool as well as the status puts two
   meanings in one channel and neither survives. So the tool gets its own
   visual channel — a tinted column band and a labelled header — and `color`
   below is for the places a tool appears ALONE: the share bar, the section
   eyebrow, a legend swatch. */
const TOOLS = [
  { key: 'truetest', label: 'TrueTest', component: 'TrueTest', color: 'var(--tool-truetest)' },
  { key: 'kse', label: 'KSE', component: 'Katalon', color: 'var(--tool-kse)' },
];
...
/* NOT A NAME IN JIRA. It is the label for the bucket of epics nobody tagged,
   and anything that turns a component into a query has to know that: the
   search for this bucket is `component IS EMPTY`, not `component = "— no
   component —"`, which matches nothing and reads as an empty suite.

   Exported for exactly that reason. It was a bare local for a long time and
   the Coverage view kept its own copy of the string to compare against —
   a magic value in two places, and the second reader to arrive got it wrong
   because there was nothing to import. */
const NO_COMPONENT = '— no component —';
/** What an epic with no Team field is called in the list of team values. */
const NO_TEAM = '— no team —';

/**
 * The product components on an epic — the tool markers are not product areas.
 *
 * `exclude` (a Set of NORMALISED names) drops components that are not
 * automation suites at all. `Technical_Works` and `KAT_Common_Maintenance` are
 * his: real Jira components holding engineering work, which carry no
 * automation status and never will, and which sit in the component picker and
 * the untriaged pile looking like suites nobody has got to yet.
 *
 * THREE OUTCOMES, AND THE THIRD IS THE ONE THAT MATTERS:
 *
 *   · some components survive → those; the excluded ones simply gone.
 *   · the epic never had any  → `— no component —`, exactly as before. An epic
 *     nobody has tagged is a real signal and must not be swept up by somebody
 *     else's exclusion list.
 *   · it HAD components and every one is excluded → `[]`, meaning not in scope
 *     at all, and the caller drops the epic. Folding it into `— no component —`
 *     instead would MOVE the work rather than remove it, and leave it sitting
 *     in the untriaged count that is the whole reason for excluding it.
 *
 * An epic carrying an excluded component AND a real suite keeps counting under
 * the real one: excluding a component removes the component, not the work.
 */
function productComponents(issue, exclude = null) {
  const real = (issue.components || []).filter(c => !TOOL_COMPONENTS.some(t => norm(t) === norm(c)));
  if (!exclude || !exclude.size) return real.length ? real : [NO_COMPONENT];
  const kept = real.filter(c => !exclude.has(norm(c)));
  if (kept.length) return kept;
  return real.length ? [] : [NO_COMPONENT];
}

/** The exclusion list as `productComponents` wants it. */
const excludeSet = (list) => new Set((list || []).map(c => norm(c)).filter(Boolean));

/**
 * WHICH EPICS A COVERAGE-SCOPED SCREEN COUNTS — the one definition.
 *
 * Two settings decide it: components that are not automation suites, and an
 * allow-list of Jira Team field values. Both are plan data, and every screen
 * that reports on this data has to honour both or the page contradicts itself.
 *
 * It lives here, apart from `view`, because `view` is not the only reader. The
 * Backlog chart counts automation EVENTS rather than statuses, so it builds its
 * own epic list — and it was building it from the raw snapshot, which meant the
 * headline said 4,141 epics while the bars below it counted 4,144. The chart's
 * drill-in did the same, so the drawer could list epics the chart above it had
 * excluded. Three readers, one rule, defined once.
 *
 * WHOSE WORK COUNTS is an ALLOW-list rather than a deny-list because the thing
 * being described is "our squads" — a fixed, short, knowable set — while the
 * other side is open-ended and grows every time somebody spins up a board. A
 * deny-list silently re-admits each new one. An epic with NO team fails the
 * test like any other, which is the "unassigned" case.
 *
 * MATCHED ON THE TEAM FIELD'S OWN VALUES, not on board names, and that
 * distinction is the entire reason this is worth a comment. His boards are
 * "Katalon Auto Titan" and "Katalon Auto Ruby"; the Team field on those same
 * epics reads "Katalon PSA (Titan)" and "Katalon RDA (Ruby)". With an
 * allow-list built from the board names, 2,158 epics of real work — more than
 * half the project — would silently leave the ratio.
 *
 * `kept` is the pool after components but BEFORE teams, because the two cuts
 * are reported separately: one number covering both leaves a reader unable to
 * tell "three engineering epics" from "eight hundred other squads'".
 */
function scoped(snap, { scope = 'Epic', exclude = null, teams = null } = {}) {
  const excluded = excludeSet(exclude);
  const allow = excludeSet(teams);
  const comps = (e) => productComponents(e, excluded);
  const teamOf = (e) => String((e && e.team) || '').trim();

  const everything = Object.values(snap.issues || {}).filter(i => norm(i.issueType) === norm(scope));
  const kept = excluded.size ? everything.filter(e => comps(e).length) : everything;
  const all = allow.size ? kept.filter(e => allow.has(norm(teamOf(e)))) : kept;

  return {
    everything, kept, all, comps, teamOf, excluded, allow,
    excludedEpics: everything.length - kept.length,
    excludedByTeam: kept.length - all.length,
  };
}

/* ── COMPONENT FAMILIES, declared once ────────────────────────────────────
   The prefix on a component name is a real division of the work — client
   delivery, product regression, the shared framework — and four screens
   filter or group by it.

   Each family owns BOTH its names: `label` is the full one a table column
   shows, `short` is the two or three characters a chip has room for.

   The older screens still derive the short form with `label.split(' —')[0]`,
   in eight places across two views — a string trick that returns the whole
   label the moment one is worded without that dash, which is exactly what
   "Other" already relies on. New work reads `short` from here instead;
   those eight are worth folding in, but not in the same change as the
   filter that needed this.

   ORDER IS READING ORDER: the two families that are almost all of the work,
   then the shared framework, then the catch-all. `Other` has no test and is
   last on purpose — it is what is left, not a rule. */
const FAMILIES = [
  { key: 'ps', short: 'PS', label: 'PS — client delivery', test: /^PS_/i },
  { key: 'rnd', short: 'R&D', label: 'R&D — product regression', test: /^R&D_/i },
  { key: 'kat', short: 'KAT', label: 'KAT — framework & common', test: /^KAT_/i },
  { key: 'other', short: 'Other', label: 'Other', test: null },
];
const OTHER_FAMILY = FAMILIES[FAMILIES.length - 1];

/** The family a component belongs to. Never null — `Other` is a real answer. */
function familyFor(component) {
  const c = String(component || '');
  return FAMILIES.find(f => f.test && f.test.test(c)) || OTHER_FAMILY;
}

/** The long label — what every existing caller already stores and groups by. */
const familyOf = (component) => familyFor(component).label;
/** The stable key, for anything that filters rather than displays. */
const familyKeyOf = (component) => familyFor(component).key;
/** The chip-sized name for a long label — the one place that mapping lives. */
const familyShort = (label) =>
  (FAMILIES.find(f => f.label === label) || { short: String(label || '') }).short;
...
/* ── ONE COMPONENT'S TOOL ROW, built in one place ─────────────────────────
   `byTool` fills these from the epics it classified. The Prioritization screen
   also needs them for components that have NO epics at all — a suite he has
   marked P1 and nobody has written an epic for is the most interesting row on
   that page, and dropping it because the tally never reached it would hide
   exactly the gap the page exists to show.

   So the shape is built here and finished here, and both callers use both
   halves. A second "blank row" literal somewhere else is how one table grows a
   `coveragePct` the other does not have, and the empty rows then render subtly
   differently from the populated ones on the same screen. */
function rawToolRow(component) {
  return {
    component, family: familyOf(component), total: 0, untagged: 0,
    ...Object.fromEntries(TOOLS.map(t => [t.key, { key: t.key, label: t.label, ...blank(), total: 0 }])),
  };
}

function finishToolRow(r) {
  return {
    ...r,
...
/** A component nothing was counted for, in the shape `byTool` rows have. */
const emptyToolRow = (component) => finishToolRow(rawToolRow(component));
 ...
 */
function view(snap, opts = {}) {
  const {
    everything, kept, all, comps, teamOf, excludedEpics, excludedByTeam,
    teamValues, componentCounts, components, selectedList, missing, selected,
    inSelection, classified, scope, asked,
  } = classify(snap, opts);

  // The population, the selection and the classification come from `classify`,
  // which `epicsIn` also runs — so every number below and every list behind
  // one of those numbers are slices of the same thing. `finish` does the
  // tallying and owns nothing else.
  return finish({
    snap, scope, comps, all, kept, everything, teamOf, excludedEpics, excludedByTeam,
    teamValues, components, selectedList, missing, selected, inSelection, classified, opts, asked,
  });
}

/**
 * THE POPULATION, THE SELECTION AND THE CLASSIFICATION — once.
 *
 * `view` renders the numbers; `epicsIn` lists the epics behind any one of
 * them. Both have to mean the same thing by "Automated in PS_iGO_NLG", and the
 * only way to guarantee that is for both to run this, rather than each
 * rebuilding the pipeline from the snapshot. A drill-in that lists a different
 * set from the number that opened it is the one failure a drill-in cannot
 * survive — the number stops being checkable, which was its whole point.
 */
function classify(snap, {
  component = null, components: wanted = null, scope = 'Epic', statusMap = STATUS_BUCKETS,
  exclude = null, teams = null,
} = {}) {
  const { everything, kept, all, comps, teamOf, excludedEpics, excludedByTeam, allow } =
    scoped(snap, { scope, exclude, teams });

  /* Every Team value in scope BEFORE the allow-list is applied, with counts —
     the list the settings screen offers, so nobody has to guess a spelling.
     Built from `kept` so the component exclusions are already honoured. */
  const teamCounts = new Map();
  for (const e of kept) {
    const t = teamOf(e) || NO_TEAM;
    teamCounts.set(t, (teamCounts.get(t) || 0) + 1);
  }
  const teamValues = [...teamCounts.entries()]
    .map(([name, count]) => ({ name, count, unassigned: name === NO_TEAM, allowed: !allow.size || allow.has(norm(name)) }))
  ...
  for (const e of all) for (const c of comps(e)) componentCounts.set(c, (componentCounts.get(c) || 0) + 1);
  ...
  /* ── the selection ──────────────────────────────────────────────────
     ...
     An epic in two selected components is counted ONCE here, because
     `inSelection` is a filter over epics rather than a union of per-component
     tallies. The
  ...
  const inSelection = chosen.size ? all.filter(e => comps(e).some(c => chosen.has(c))) : all;
  ...
  const classified = inSelection.map(issue => ({ issue, bucket: bucketOf(issue, statusMap), tool: toolOf(issue) }));

  return {
    everything, kept, all, comps, teamOf, allow, excludedEpics, excludedByTeam,
    teamValues, componentCounts, components, selectedList, missing, selected,
    inSelection, classified, scope, asked,
  };
}
 ...
 *
 * A slice of the same classification the screen counted: `component` narrows
 * to one grid row, `bucket` to one column, `tool` to one half of the TrueTest
 * vs KSE split. Leave one out and it does not narrow — so no arguments at all
 * is the headline's own population, which is exactly what the KPI strip shows.
 *
 * The selection, the excluded components and the team allow-list are already
 * applied, because they were applied by `classify` before anything was counted.
 */
function epicsIn(snap, opts = {}, { component = null, buckets = null, tool = null } = {}) {
  const { classified, comps } = classify(snap, opts);
  const want = component == null ? null : norm(component);
  // A LIST of columns, OR'd, because one number on the screen is the sum of
  // two of them: "Still to automate" is Ready plus Blocked. An empty list does
  // not narrow, so the headline's own population is the default.
  const cols = new Set((buckets == null ? [] : [].concat(buckets)).filter(Boolean));
  return classified
    .filter(c => (!cols.size || cols.has(c.bucket)))
    .filter(c => (tool == null || c.tool === tool))
    .filter(c => (want == null || comps(c.issue).some(x => norm(x) === want)))
    .map(c => ({ ...c.issue, bucket: c.bucket, tool: c.tool }));
}

/** The rest of `view` — the tallies, over an already-classified population. */
function finish({
  snap, scope, comps, all, kept, everything, teamOf, excludedEpics, excludedByTeam,
  teamValues, components, selectedList, missing, selected, inSelection, classified, opts, asked,
}) {
  const statusMap = opts.statusMap || STATUS_BUCKETS;
  const counts = blank();
  ...
  for (const { issue, bucket, tool } of classified) {
    for (const c of comps(issue)) {
      if (!toolRows.has(c)) toolRows.set(c, rawToolRow(c));
  ...
  return {
    ...
    /* WHAT WAS LEFT OUT, AND HOW MUCH OF IT. A denominator that quietly got
       smaller is the one thing this screen must never do — every percentage on
       it is read against that number, and a reader who cannot see the
       exclusion cannot check the figure. Both travel in the payload so the
       screen can say it in words. */
    excludedComponents: (opts.exclude || []).filter(Boolean),
    excludedEpics,
    // The team side of the same promise: which squads count, how many epics
    // the rule removed, and every value present so the setting can be chosen
    // from the data instead of typed from memory.
    coverageTeams: (opts.teams || []).filter(Boolean),
    excludedByTeam,
    teamValues,
...
module.exports = {
  view, assess, bucketOf, toolOf, hasTool, productComponents, excludeSet, familyOf, scoped, classify, epicsIn, NO_TEAM,
  emptyToolRow, FAMILIES, familyFor, familyKeyOf, familyShort, NO_COMPONENT,
