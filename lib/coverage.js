...
function view(snap, {
  ...
  /* EXCLUSION HAPPENS ONCE, HERE. Five places below ask an epic for its
     components, and an exclusion applied in four of them is a screen whose
     headline and grid disagree. `comps` is the only way this function reads a
     component from now on, and `pool` the only set of epics it counts. */
  ...
  /* ── WHOSE WORK COUNTS ───────────────────────────────────────────────
     An ALLOW-list of Jira Team field values. Empty means count everything,
     which is the behaviour this screen has always had; non-empty means an
     epic counts only if its Team is one of them, and an epic with NO team
     fails that test like any other.

     It is an allow-list rather than a deny-list because the thing being
     described is "our squads" — a fixed, short, knowable set — while the
     other side is open-ended and grows every time somebody spins up a board.
     A deny-list would silently re-admit each new one.
     ...
     allow-list built from the board names, 2,158 epics of real work — more
     than half the project — would silently leave the ratio. So the screen
     offers the values that are actually in the data, with their counts, and
     the reader picks from those rather than typing from memory. */
  const allow = excludeSet(teams);
  const teamOf = (e) => String((e && e.team) || '').trim();
  const inTeam = (e) => allow.has(norm(teamOf(e)));

  const kept = excluded.size ? everything.filter(e => comps(e).length) : everything;
  const all = allow.size ? kept.filter(inTeam) : kept;
  /* The two cuts are counted SEPARATELY. One number covering both would leave
     a reader unable to tell "three engineering epics" from "eight hundred
     other squads'", which are very different facts about the same total. */
  const excludedEpics = everything.length - kept.length;
  const excludedByTeam = kept.length - all.length;
  ...
  const scoped = chosen.size ? all.filter(e => comps(e).some(c => chosen.has(c))) : all;
...
module.exports = {
  view, assess, bucketOf, toolOf, hasTool, productComponents, excludeSet, familyOf, NO_TEAM,
