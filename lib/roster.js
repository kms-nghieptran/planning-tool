...
function baseFor(plan, team, sprint, issues, state, excluded) {
  ...
  for (const { id, member, aliasIds } of plannedFor(plan, team, sprint)) {
    ...
    if (existing) {
      // Already here as an assignee. This is a MERGE, not a second row: the
      // same person whose capacity data happens to be filed under an older id.
      // Appending a row instead doubled his Ruby S33 from 4 people and 231
      // hours to 8 and 462 — a worse failure than the one this fixes.
      existing.aliasIds = [...new Set([...(existing.aliasIds || []), ...aliasIds])];
      continue;
    }
...
function forSprint(plan, team, sprint, issues, state = null) {
  const excluded = excludedSet(plan, team.id);
