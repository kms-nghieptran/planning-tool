...
function buildTeamIndex(plan, snapshot) {
  ...
  for (const team of plan.teams) {
    ...
    const keywords = (team.sprintKeywords && team.sprintKeywords.length ? team.sprintKeywords : [team.name]).map(norm);
...
function reconcileMembers(plan, snapshot, { autoAdd = true } = {}) {
  ...
  for (const team of plan.teams) {
    ...
    // 3. Active members Jira has never seen — nearly always a name mismatch.
 ...
 */
function addMember(plan, teamId, person, { source } = {}) {
  const team = plan.teams.find(t => t.id === teamId);
  if (!team) throw new Error(`No team "${teamId}"`);
  ...
  team.members.push({
    ...
    status: 'Active',
    supportPct: 0,
    jiraAccountId: person.accountId || null,
    source: source || (person.accountId ? 'jira' : 'manual'),
