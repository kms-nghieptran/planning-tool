...
function ruleHits(issues, rules) {
  ...
  const hits = new Map(list.map(r => [r.id, 0]));
  let unmatched = 0;
  for (const issue of issues || []) {
    const winner = list.find(r => matches(issue, r));
    if (winner) hits.set(winner.id, (hits.get(winner.id) || 0) + 1);
    else unmatched++;
  }
  return { byRule: Object.fromEntries(hits), unmatched, total: (issues || []).length };
}

module.exports = {
  CATEGORIES, DEFAULT_RULES, DEFAULT_TARGETS, classify, mix, mixVsTarget, matches,
  FIELDS, OPS, validateRules, ruleHits,
};
