...
function plan() {
  ...
  return {
    ...
    // A row means exempt — see the table's own note. `true` rather than the
    // row so nothing downstream can mistake an object for a percentage.
    calcExempt: keyed('SELECT team_id, sprint_id, member_id FROM calc_exempt', () => true),
    ...
    /* Sprint spans he has corrected by hand, keyed by sprint id. Plan data,
       so a sync never reaches them — Jira's own dates stay in the snapshot as
       the record of what it said. */
    sprintDates: s['plan.sprintDates'] ?? {},
    componentPriority: s['plan.componentPriority'] ?? {},
    /* What he wants remembered about a component, in his own words — the Notes
       column on the Prioritization screen. Plan data for exactly the reason a
       priority is: nothing in Jira can say it, so a sync must never reach it.
       A settings blob rather than a table because it is keyed by component name
       and read whole, which is the shape componentPriority above already has —
       and a second storage shape for the same key would be a migration and a
       second set of round-trip rules for no gain. */
    componentNote: s['plan.componentNote'] ?? {},
