...
function clearImportedTables() {
  for (const t of [
    ...
    'availability', 'support_pct', 'calc_exempt', 'ceremony', 'plan_override', 'plan_note',
...
function importPlan(plan) {
  ...
  // Only the truthy ones: a row IS the exemption, so a false must not create one.
  out.calcExempt = keyed(plan.calcExempt, (team, sprint, member, v) => {
    if (v) db.run('INSERT OR REPLACE INTO calc_exempt VALUES (?, ?, ?)', team, sprint, member);
  });
  ...
  for (const key of ['categoryRules', 'mixTargets', 'componentPriority', 'componentNote', 'excludedComponents', 'coverageTeams', 'sprintDates', 'ignoredBoards', 'version', 'updatedAt']) {
