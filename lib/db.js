...
const MIGRATIONS = [
  ...
  },
  {
     ...
     *
     * Not the same as any of the three things that already remove someone.
     * "status = 'Released'" says they have LEFT. plan.excluded says they
     * were never this team's. The per-sprint roster says they are not on THIS
     * sprint. This one says: they are here, they may well be carrying work,
     * and their hours should not be counted when planning how much the team
     * can take on — a lead whose time is meetings, someone lent out for a
     * fortnight, a contractor whose capacity is budgeted elsewhere.
     *
     * PER SPRINT, keyed exactly like support_pct beside it, because every
     * other per-person planning decision on that screen is per sprint and
     * being lent out is a thing that happens for a fortnight.
     *
     * A ROW MEANS EXEMPT. There is no boolean column: clearing the toggle
     * deletes the row, so "has a value" and "is exempt" cannot come apart the
     * way a table full of zeroes lets them. */
    CREATE TABLE calc_exempt (
      team_id   TEXT NOT NULL,
      sprint_id TEXT NOT NULL,
      member_id TEXT NOT NULL,
      PRIMARY KEY (team_id, sprint_id, member_id)
